...
function renderWalletDetail(d){
  ...
  html+='<div class="ws-section"><h3>🔐 LIVE Wallet Credentials</h3>'+
    ...
    '<div class="ws-field"><label>Auth Mode</label><input type="text" value="CLOB L1+L2 (private key, plus API creds from env or auto-derive)" disabled></div>'+
    '<div class="ws-field"><label>L2 Credentials Status</label><input type="text" value="'+(liveStatus.l2HeadersConfigured?'Configured':'Will auto-derive from private key at runtime')+'" disabled></div>'+
    '<div class="ws-field"><label>Live Readiness</label><input type="text" value="'+((liveStatus.privateKeyConfigured&&liveStatus.walletAddressConfigured)?'Ready':'Missing private key or wallet/proxy address')+'" disabled></div>'+
    '</div>'+
