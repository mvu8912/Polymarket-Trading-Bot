...
function renderWalletDetail(d){
  ...
  html+='<div class="ws-section"><h3>🔐 LIVE Wallet Credentials</h3>'+
    ...
    '<div class="ws-field"><label>Auth Mode</label><input type="text" value="CLOB requires L1+L2 (private key + API creds)" disabled></div>'+
    '<div class="ws-field"><label>L2 Credentials Status</label><input type="text" value="'+(liveStatus.l2HeadersConfigured?'Configured':'Missing POLY_API_KEY/POLY_PASSPHRASE/POLY_SECRET')+'" disabled></div>'+
    '<div class="ws-field"><label>Live Readiness</label><input type="text" value="'+((liveStatus.privateKeyConfigured&&liveStatus.l2HeadersConfigured)?'Ready':'Missing private key or L2 credentials')+'" disabled></div>'+
    '</div>'+
